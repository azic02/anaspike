...
class SpatialGridMap(Generic[GridT, ValT]):
    def __init__(self, grid: GridT, elements: NDArray[ValT]):
        if elements.ndim < 2:
    ...
    @property
    def elements(self) -> NDArray[ValT]:
...
def calculate_fft_2d(gf: SpatialGridMap[RegularGrid2D,ValT]):
...
def calculate_ifft_2d(gf: SpatialGridMap[RegularGrid2D,ValT]):
...
def calculate_psd_2d(gf: SpatialGridMap[RegularGrid2D,ValT]) -> SpatialGridMap[RegularGrid2D,np.float64]:
    fft = calculate_fft_2d(gf)
    psd_elements = np.abs(fft.elements) ** 2
    return SpatialGridMap(fft.grid, psd_elements)


def calculate_autocorrelation_2d_wiener_khinchin(gf: SpatialGridMap[RegularGrid2D,ValT]) -> SpatialGridMap[RegularGrid2D,np.float64]:
