...
class SpikeTrains(HDF5Mixin):
    ...
    def __getitem__(self, index: Union[int, slice, NDArray[np.int64]]) -> Union['SpikeTrains', SpikeTrain]:
        if isinstance(index, int):
            return np.asarray(self._spike_trains[index], dtype=np.float64)
        return SpikeTrains(self._spike_trains[index])
