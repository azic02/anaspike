...
class TestSpikeCountsFromSpikeTrains(unittest.TestCase):
    ...
    def test_all_time(self):
        counts = SpikeCounts.from_spike_trains(self.spike_trains, self.t_interval_all)
        np.testing.assert_array_equal(counts, np.array([5, 3, 0]))

    def test_partial_time(self):
        counts = SpikeCounts.from_spike_trains(self.spike_trains, self.t_interval_partial)
        np.testing.assert_array_equal(counts, np.array([2, 3, 0]))

    def test_no_spikes(self):
        counts = SpikeCounts.from_spike_trains(self.spike_trains, self.t_interval_empty)
        np.testing.assert_array_equal(counts, np.array([0, 0, 0]))

    def test_single_neuron(self):
        coords = Coords2D([0], [0])
        spike_train = SpikeTrains(coords, [self.spike_trains[0]])
        counts = SpikeCounts.from_spike_trains(spike_train, self.t_interval_partial)
